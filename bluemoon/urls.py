...
from django.db.models import Sum, Count, Q, OuterRef, Subquery, Exists, F, Value, Min, DecimalField
...
def cudan_view(request):
    ...
    fees = Fee.objects.filter(
        (Q(is_common=True) | Q(households=household)) & Q(due_date__month=today.month)
    ).distinct()
